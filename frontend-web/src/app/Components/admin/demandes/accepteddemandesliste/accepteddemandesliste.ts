...
@Component({
  selector: 'app-accepteddemandesliste',
  standalone: true,
  imports: [CommonModule, RouterModule,Alert],
  templateUrl: './accepteddemandesliste.html',
  styleUrl: './accepteddemandesliste.css'
})
export class Accepteddemandesliste implements OnInit {
  demandes: any[] = [];
    ...
    this.adminService.getDemandes().subscribe({
      next: (res: any) => {
        ...
        this.filteredDemandes = this.demandes.filter(
          (demande) => demande.etat_demande === 'acceptée'
        );
